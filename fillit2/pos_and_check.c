...
/*   Updated: 2017/12/15 19:58:12 by mmanley          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		tab_checker(t_forme *ptr, char **tab)
{
	...
	while (tab[y])
	{
		while (tab[y][x] && tab[y][x] != ptr->next->myletter)
			x++;
		if (tab[y][x] && tab[y][x] == ptr->next->myletter)
		{
			printf("%s, %c, %d\n", tab[y], tab[y][x], x + (y * 4));
			return (x + (y * 4));
		}
...
int		check_posabilty(t_forme *ptr, int position, char **tab, char **newtab)
{
	t_coor lco;
	t_coor t_co;
	int i;
...
int		check_posabilty_test(t_forme *ptr, t_coor co, char **tab, char **newtab)
{
	t_coor lco;
	t_coor t_co;
	int i;
...
int		pos_tetriminos(t_forme *ptr, char **newtab, t_coor co)
{
	...
	tmp = co.x;
//	printf("Yo, (%d, %d)\n", co.y, co.x);
	if (check_posabilty_test(ptr, co, ptr->next->tab, newtab) == 1)
	{
	//	printf("Je\n");
