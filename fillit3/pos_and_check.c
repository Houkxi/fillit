...
/*   Updated: 2017/12/19 17:42:10 by mmanley          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			chk_all_psblty(t_forme *ptr, int size, char **tab, char **newt)
{
	...
	while (i < size * size - 1)
	{
		if (ptr->next && (check_posabilty(ptr, i, tab, newt)) == 1)
			return (1);
		i++;
	}
	return (0);
...
int			check_posabilty(t_forme *ptr, int pos, char **tab, char **newt)
{
	...
	while (tab[lco.y])
	{
		...
		while (tab[lco.y][lco.x] && tab[lco.y][lco.x] == ptr->next->myl)
		{
			...
			if (t_co.y < 0 || t_co.x < 0 || t_co.y >= (int)ft_strlen(*newt)
			|| t_co.x >= (int)ft_strlen(*newt) || newt[t_co.y][t_co.x] != '.')
				return (0);
			j++;
			t_co.y = pos / i;
			t_co.x = pos % i;
...
int			pos_tetriminos(t_forme *pt, char **newt, int pos, int size)
{
	...
	i = 0;
	tmp = co.x;
	if (check_posabilty(pt, pos, pt->next->tab, newt) == 1)
	{
		while (pt->next->tab[y])
		{
